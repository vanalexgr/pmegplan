...
export function buildBenchCtRenderModel(
  ...
  const rings = descriptor.rings.map((ring) => ({
    index: ring.index,
  ...
  return {
    ...
    fabricStartMm,
    fabricEndMm,
    fabricLengthMm: Math.max(fabricEndMm - fabricStartMm, 1),
    minimumZMm,
    rings,
    barbs,
    diameterAt: (zMm) => interpolateDiameter(descriptor, zMm + fabricStartMm),
  };
}
