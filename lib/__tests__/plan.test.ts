...
describe("planGraft", () => {
  ...
  it("plans an out-of-window device rather than refusing, and flags it", () => {
    ...
    // Set aside anything refused for a reason of its own, so what is left tests
    // oversizing alone rather than whatever else the device is short of.
    const flagged = plan.considered.filter(
      (fit) => fit.oversizeOutOfRange !== null && fit.rejection === null,
    );
